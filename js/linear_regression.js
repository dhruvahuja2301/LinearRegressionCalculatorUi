...
function calculate_simple_regression() {
    ...
    input1 = get_columns("tab1", 0, 1), (r = input1.table)[0].length != r[1].length && alert("Please enter equal number of X's and Y's. You entered X:" + r[0].length + ", Y:" + r[1].length), n ? display_blocks("calculation1", "block") : display_blocks("calculation1", "none");
...
function anova_table(e, s, r, a, t, o, n, u, l, i, b, p, d, h, c, g) {
    ...
    var I = b_tooltip(xround(p, h), S, D),
        Y = [
            ["Source", b_tooltip("DF", "Degrees of Freedom"), "Sum of Square", "Mean Square", "F Statistic (df<sub>1</sub>,df<sub>2</sub>)", "P-value"],
            [F, b_tooltip(Math.round(n, h), _), b_tooltip(xround(a, h), v), b_tooltip(xround(l, h), m), b_tooltip(xround(b, h) + " (" + n + "," + u + ")", f), I],
            [R, b_tooltip(Math.round(u, h), x), b_tooltip(xround(t, h), y), b_tooltip(xround(i, h), w), "", ""],
            [E, b_tooltip(Math.round(o, h), M), b_tooltip(xround(r, h), T), b_tooltip(xround(r / o, h), k), "", ""]
        ];
